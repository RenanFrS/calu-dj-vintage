...
export type MasonryImage = string | { src: string; objectPosition?: string };

type MasonryProps = React.HTMLAttributes<HTMLElement> & { images?: MasonryImage[] };

function normalize(image: MasonryImage | undefined): { src: string; objectPosition?: string } | null {
      ...
      <section {...rest} className={`relative ${className ?? "py-12"}`}>
        <div className="container mx-auto px-4">
          <Card className="p-12 text-center border-2 border-dashed border-secondary/30 bg-card/50">
            ...
            <p className="text-sm text-muted-foreground mt-2">Adicione imagens na coleção "Galeria"</p>
          </Card>
        </div>
      </section>
    );
...
const MasonryGallery: React.FC<MasonryProps & { images: MasonryImage[] }> = ({ images: imgs, className, ...rest }) => {
        ...
        return (
          ...
          >
            <img
              src={image.src}
              alt={`gallery-${i}`}
              className="pointer-events-none object-cover w-full h-full block"
              style={image.objectPosition ? { objectPosition: image.objectPosition } : undefined}
            />
          </div>
        );
